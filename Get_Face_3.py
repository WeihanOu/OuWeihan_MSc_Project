...
file_lin = 'D:/PycharmWorkSpace/MScProj/data/217-Ju.dat'


def prepare_data(img_frames, time_arr):
    # split train and test set
    time_arr = time_arr[1:]
    relex_begin = np.amin(np.where(time_arr > 1))
    relex_over = np.amin(np.where(time_arr > 92))
    stress_1_1_begin = np.amin(np.where(time_arr > 243))
    stress_1_1_over = np.amin(np.where(time_arr > 335))
    stress_1_2_begin = np.amin(np.where(time_arr > 386))
    stress_1_2_over = np.amin(np.where(time_arr > 440))
    stress_2_1_begin = np.amin(np.where(time_arr > 485))
    stress_2_1_over = np.amin(np.where(time_arr > 563))
    stress_2_2_begin = np.amin(np.where(time_arr > 780))
    stress_2_2_over = np.amin(np.where(time_arr > 809))
...
def extract_face(x_train):
    ...
    plt.imsave('./data/faces/ju/x_mean.png', x_train_mean)
    x_train = x_train[0:194, :, :]
    # x_test = x_test[0:204, :, :]
    x_train_mean = np.mean(x_train, 2)
    plt.imsave('./data/faces/ju/face_mean.png', x_train_mean)
    # get four lines to cut the face from data_frames
    x_train_mean_2d = np.reshape(x_train_mean, (194*240))
    top = int(np.floor(np.amin(np.where(x_train_mean_2d > 32))/240))
    bottom = int(np.floor(np.amax(np.where(x_train_mean_2d > 32)) / 240))
    x_train_mean_2d = np.reshape(np.transpose(x_train_mean), (194 * 240))
    left = int(np.floor(np.amin(np.where(x_train_mean_2d > 31.8)) / 194))
    right = int(np.floor(np.amax(np.where(x_train_mean_2d > 32.5)) / 194))
    x_train_mean_cut = x_train_mean[top:bottom, left:right]
    plt.imsave('./data/faces/ju/face_mean.png', x_train_mean_cut)
    face_hight = bottom-top
    face_width = right-left
    # cut train data_frames
    x_train_cut = np.zeros((face_hight,face_width, np.size(x_train, 2)))
    test = x_train[:,:,-1]
    plt.imsave('./data/faces/ju/000' + str(0) + '.png', test)
    for i in range(np.size(x_train, 2)):
        this_face = np.reshape(x_train[:, :, i], (194 * 240))
        this_top = int(np.floor(np.amin(np.where(this_face > 32)) / 240))
        if (this_top+face_hight) > 194:
            this_top -= (np.size(this_face, 0)-194)
        this_face = np.reshape(np.transpose(x_train[:, :, i]), (194 * 240))
        this_left = int(np.floor(np.amin(np.where(this_face > 31.8)) / 194))
        x_train_cut[:, :, i] = x_train[top:bottom, this_left:this_left+face_width, i]
        plt.imsave('./data/faces/ju/train_' + str(i) + '.png', x_train_cut[:, :, i])
