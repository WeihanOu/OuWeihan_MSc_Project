import numpy as np
from ReadImg import *
from ExtractFeature import *
import random
from sklearn.svm import LinearSVC


file_lin = 'D:/PycharmWorkSpace/MScProj/data/407-Nadia.dat'


def prepare_data(img_frames, time_arr):

    # split train and test set
    time_arr = time_arr[1:]
    relex_begin = np.amin(np.where(time_arr > 2))
    relex_over = np.amin(np.where(time_arr > 92))
    stress_1_1_begin = np.amin(np.where(time_arr > 122))
    stress_1_1_over = np.amin(np.where(time_arr > 218))
    stress_1_2_begin = np.amin(np.where(time_arr > 300))
    stress_1_2_over = np.amin(np.where(time_arr > 395))
    stress_2_1_begin = np.amin(np.where(time_arr > 570))
    stress_2_1_over = np.amin(np.where(time_arr > 650))
    stress_2_2_begin = np.amin(np.where(time_arr > 683))
    stress_2_2_over = np.amin(np.where(time_arr > 759))
    stress_3_1_begin = np.amin(np.where(time_arr > 828))
    stress_3_1_over = np.amin(np.where(time_arr > 880))
    relax = relex_over-50-relex_begin
    stress1 = stress_1_1_over-stress_1_1_begin+stress_1_2_over-50-stress_1_2_begin
    stress2 = stress_2_1_over-stress_2_1_begin+stress_2_2_over-50-stress_2_2_begin+stress_3_1_over-stress_3_1_begin
    """" new version do not distinguish train /test"""
    x_train = np.concatenate((img_frames[:, :, relex_begin:relex_over],
                              img_frames[:, :, stress_1_1_begin:stress_1_1_over],
                              img_frames[:, :, stress_1_2_begin:stress_1_2_over],
                              img_frames[:, :, stress_2_1_begin:stress_2_1_over],
                              img_frames[:, :, stress_2_2_begin:stress_2_2_over],
                              img_frames[:, :, stress_3_1_begin:stress_3_1_over]), axis=2)

    """" old version distinguish train/test for individual analysis"""
    # x_train = np.concatenate((img_frames[:, :, relex_begin:relex_over-50],
    #                           img_frames[:, :, stress_1_1_begin:stress_1_1_over],
    #                           img_frames[:, :, stress_1_2_begin:stress_1_2_over-50],
    #                           img_frames[:, :, stress_2_1_begin:stress_2_1_over],
    #                           img_frames[:, :, stress_2_2_begin:stress_2_2_over-50],
    #                           img_frames[:, :, stress_3_1_begin:stress_3_1_over]), axis=2)
    # x_test = np.concatenate((img_frames[:, :, relex_over-50:relex_over],
    #                          img_frames[:, :, stress_1_2_over-50:stress_1_2_over],
    #                          img_frames[:, :, stress_2_2_over-50:stress_2_2_over]), axis=2)
    # # for i_save in range(np.size(x_test, 2)):
    # #     this_frame = x_test[:, :, i_save]
    # #     plt.imsave('./data/test_ou_' + str(i_save) + '.png', this_frame)
    # # apply labels to train & test set
    # y_train = np.zeros(np.size(x_train, 2))
    # y_train[0:relex_over-50-relex_begin] = 0
    # y_train[(relex_over-50-relex_begin):(relex_over-50-relex_begin) + (stress_1_1_over-stress_1_1_begin) + (stress_1_2_over-50-stress_1_2_begin)] = 1
    # y_train[(relex_over-50-relex_begin) + (stress_1_1_over-stress_1_1_begin) + (stress_1_2_over-50-stress_1_2_begin):
    #         (relex_over-50-relex_begin)+(stress_1_1_over-stress_1_1_begin)+(stress_1_2_over-50-stress_1_2_begin)+
    #         (stress_2_1_over - stress_2_1_begin) + (stress_2_2_over - 50 - stress_2_2_begin)+stress_3_1_over-stress_3_1_begin] = 2
    """new version"""
    y_train = np.zeros(np.size(x_train, 2))
    y_train[0:relex_over-relex_begin] = 0
    y_train[(relex_over-relex_begin):(relex_over-relex_begin) + (stress_1_1_over-stress_1_1_begin) + (stress_1_2_over-stress_1_2_begin)] = 1
    y_train[(relex_over-relex_begin) + (stress_1_1_over-stress_1_1_begin) + (stress_1_2_over-stress_1_2_begin):
            (relex_over-relex_begin)+(stress_1_1_over-stress_1_1_begin)+(stress_1_2_over-stress_1_2_begin)+
            (stress_2_1_over - stress_2_1_begin) + (stress_2_2_over - stress_2_2_begin)+stress_3_1_over-stress_3_1_begin] = 2
    # y_test = np.zeros(150)
    # y_test[0:50] = 0
    # y_test[50:100] = 1
    # y_test[100:150] = 2
    # return x_train, x_test, y_train, y_test
    return x_train, y_train


def extract_face(x_train):
    # cut no-use portion & calculate mean
    # x_train = x_train[15:, :, :]
    # x_test = x_test[15:, :, :]
    # img_frames = np.rot90(img_frames, 2)
    for i in range(np.size(x_train, 2)):
        x_train[:, :, i] = np.rot90(x_train[:, :, i], 2)
    # for i in range(np.size(x_test, 2)):
    #     x_test[:, :, i] = np.rot90(x_test[:, :, i], 2)
    x_train_mean = np.mean(x_train, 2)
    plt.imsave('./data/faces/nadia/x_mean.png', x_train_mean)
    x_train = x_train[0:204, :, :]
    # x_test = x_test[0:204, :, :]
    x_train_mean = np.mean(x_train, 2)
    plt.imsave('./data/faces/nadia/face_mean.png', x_train_mean)
    # get four lines to cut the face from data_frames
    x_train_mean_2d = np.reshape(x_train_mean, (204*240))
    top = int(np.floor(np.amin(np.where(x_train_mean_2d > 33))/240))
    bottom = int(np.floor(np.amax(np.where(x_train_mean_2d > 32)) / 240))
    x_train_mean_2d = np.reshape(np.transpose(x_train_mean), (204 * 240))
    left = int(np.floor(np.amin(np.where(x_train_mean_2d > 32)) / 204))
    right = int(np.floor(np.amax(np.where(x_train_mean_2d > 32)) / 204))
    x_train_mean_cut = x_train_mean[top:bottom, left:right]
    plt.imsave('./data/faces/nadia/face_mean.png', x_train_mean_cut)
    face_hight = bottom-top
    face_width = right-left
    # cut train data_frames
    x_train_cut = np.zeros((face_hight,face_width, np.size(x_train, 2)))
    test = x_train[:,:,-1]
    plt.imsave('./data/faces/nadia/000' + str(0) + '.png', test)
    for i in range(np.size(x_train, 2)):
        this_face = np.reshape(np.transpose(x_train[:, :, i]), (204 * 240))
        this_left = int(np.floor(np.amin(np.where(this_face > 33)) / 204))
        x_train_cut[:, :, i] = x_train[top:bottom, this_left:this_left+face_width, i]
        # plt.imsave('./data/faces/nadia/train_' + str(i) + '.png', x_train_cut[:, :, i])
    # cut test data_frames
    # x_test_cut = np.zeros((face_hight, face_width, np.size(x_test, 2)))
    # for i in range(np.size(x_test, 2)):
    #     this_face = np.reshape(np.transpose(x_test[:, :, i]), [204*240])
    #     this_left = int(np.floor(np.amin(np.where(this_face > 33)) / 204))
    #     x_test_cut[:, :, i] = x_test[top:bottom, this_left:this_left + face_width, i]
    #     # plt.imsave('./data/faces/nadia/test_' + str(i) + '.png', x_test_cut[:, :, i])
    # return x_train_cut, x_test_cut, abs(top-bottom), (right-left)
    return x_train_cut, abs(top-bottom), (right-left)


"""old version"""
# def extract_face(x_train, x_test):
#     # cut no-use portion & calculate mean
#     # x_train = x_train[15:, :, :]
#     # x_test = x_test[15:, :, :]
#     # img_frames = np.rot90(img_frames, 2)
#     for i in range(np.size(x_train, 2)):
#         x_train[:, :, i] = np.rot90(x_train[:, :, i], 2)
#     for i in range(np.size(x_test, 2)):
#         x_test[:, :, i] = np.rot90(x_test[:, :, i], 2)
#     x_train_mean = np.mean(x_train, 2)
#     plt.imsave('./data/faces/nadia/x_mean.png', x_train_mean)
#     x_train = x_train[0:204, :, :]
#     x_test = x_test[0:204, :, :]
#     x_train_mean = np.mean(x_train, 2)
#     plt.imsave('./data/faces/nadia/face_mean.png', x_train_mean)
#     # get four lines to cut the face from data_frames
#     x_train_mean_2d = np.reshape(x_train_mean, (204*240))
#     top = int(np.floor(np.amin(np.where(x_train_mean_2d > 33))/240))
#     bottom = int(np.floor(np.amax(np.where(x_train_mean_2d > 32)) / 240))
#     x_train_mean_2d = np.reshape(np.transpose(x_train_mean), (204 * 240))
#     left = int(np.floor(np.amin(np.where(x_train_mean_2d > 32)) / 204))
#     right = int(np.floor(np.amax(np.where(x_train_mean_2d > 32)) / 204))
#     x_train_mean_cut = x_train_mean[top:bottom, left:right]
#     plt.imsave('./data/faces/nadia/face_mean.png', x_train_mean_cut)
#     face_hight = bottom-top
#     face_width = right-left
#     # cut train data_frames
#     x_train_cut = np.zeros((face_hight,face_width, np.size(x_train, 2)))
#     test = x_train[:,:,-1]
#     plt.imsave('./data/faces/nadia/000' + str(0) + '.png', test)
#     for i in range(np.size(x_train, 2)):
#         this_face = np.reshape(np.transpose(x_train[:, :, i]), (204 * 240))
#         this_left = int(np.floor(np.amin(np.where(this_face > 33)) / 204))
#         x_train_cut[:, :, i] = x_train[top:bottom, this_left:this_left+face_width, i]
#         # plt.imsave('./data/faces/nadia/train_' + str(i) + '.png', x_train_cut[:, :, i])
#     # cut test data_frames
#     x_test_cut = np.zeros((face_hight, face_width, np.size(x_test, 2)))
#     for i in range(np.size(x_test, 2)):
#         this_face = np.reshape(np.transpose(x_test[:, :, i]), [204*240])
#         this_left = int(np.floor(np.amin(np.where(this_face > 33)) / 204))
#         x_test_cut[:, :, i] = x_test[top:bottom, this_left:this_left + face_width, i]
#         # plt.imsave('./data/faces/nadia/test_' + str(i) + '.png', x_test_cut[:, :, i])
#     return x_train_cut, x_test_cut, abs(top-bottom), (right-left)


def get_face_nadia():
    # read img & prepossessing
    file = read_file(file_lin, 320, 240)
    img_frames = get_img_frames(file, 320, 240)
    # prepare face_data
    time_arr = get_time_arr(file)  #246.149s
    """ old version"""
    # x_train, x_test, y_train, y_test = prepare_data(img_frames, time_arr)
    """ new version"""
    x_train, y_train = prepare_data(img_frames, time_arr)
    # face_train, face_test, h, w = extract_face(x_train, x_test)
    face_train, h, w = extract_face(x_train)
    # img_frames = img_frames/np.mean(img_frames)
    # return face_train, face_test, y_train, y_test, h, w
    # eigen_faces, x_train_pca, x_test_pca, var_percent = pca(face_train, face_test, h, w)
    # for i_save in range(np.size(eigen_faces, 0)):
    #     this_eg = eigen_faces[i_save, :]
    #     plt.imsave('./data/eigen/nadia/' + str(i_save) + '.png', (np.reshape(this_eg, [h, w])))
    # print(var_percent)
    # print(sum(var_percent))
    # classifier = LinearSVC()
    # classifier.fit(x_train_pca, y_train)
    # train_accuracy = classifier.score(x_train_pca, y_train)
    # test_accuracy = classifier.score(x_test_pca, y_test)
    # print(train_accuracy)
    # print(test_accuracy)

    # return face_train, face_test, y_train, y_test, h, w
    return face_train, y_train, h, w
if __name__ == '__main__':
    get_face_nadia()



