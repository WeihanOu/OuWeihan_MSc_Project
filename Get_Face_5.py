...
file_lin = 'D:/PycharmWorkSpace/MScProj/data/932-lin.dat'
...
def prepare_data(img_frames, time_arr):
    ...
    stress_1_1_over = np.amin(np.where(time_arr > 230))
    stress_1_2_begin = np.amin(np.where(time_arr > 257))
    stress_1_2_over = np.amin(np.where(time_arr > 351))
    stress_2_1_begin = np.amin(np.where(time_arr > 390))
    stress_2_1_over = np.amin(np.where(time_arr > 467))
    stress_2_2_begin = np.amin(np.where(time_arr > 491))
    stress_2_2_over = np.amin(np.where(time_arr > 562))
...
#     plt.imsave('./data/faces/lin/x_mean.png', x_train_mean)
#     x_train = x_train[0:229, :, :]
#     x_train_mean = np.mean(x_train, 2)
#     plt.imsave('./data/faces/lin/x_mean.png', x_train_mean)
#     # get four lines to cut the face from data_frames
#     x_train_mean_2d = np.reshape(x_train_mean, (229*240))
#     top = int(np.floor(np.amin(np.where(x_train_mean_2d > 17))/240))
#     bottom = int(np.floor(np.amax(np.where(x_train_mean_2d > 17)) / 240))
#     x_train_mean_2d = np.reshape(np.transpose(x_train_mean), (229 * 240))
#     left = int(np.floor(np.amin(np.where(x_train_mean_2d > 15)) / 229))
#     right = int(np.floor(np.amax(np.where(x_train_mean_2d > 15)) / 229))
#     x_train_mean_cut = x_train_mean[top:bottom, left:right]
#     plt.imsave('./data/faces/lin/face_mean.png', x_train_mean_cut)
...
#     #     plt.imsave('./data/faces/lin/train_' + str(i_save) + '.png', this_frame)
#     # cut test data_frames
#     x_test_cut = x_test[top:bottom, left:right, :]
#     # for i_save in range(np.size(x_test, 2)):
#     #     this_frame = x_test_cut[:, :, i_save]
#     #     plt.imsave('./data/faces/lin/test_' + str(i_save) + '.png', this_frame)
...
def get_face_lin():
    ...
    #     plt.imsave('./data/eigen/lin/' + str(i_save) + '.png', (np.reshape(this_eg, [h, w])))
    # print(var_percent)
