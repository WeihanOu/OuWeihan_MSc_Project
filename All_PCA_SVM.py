...
def choose_img(x):
    ...
    idx_9_0 = np.reshape(random.sample(range(0, 757), n_img), [-1, 1])
    idx_9_1 = np.reshape(random.sample(range(757, 2450), n_img), [-1, 1])
    idx_9_2 = np.reshape(random.sample(range(2450, np.size(x[9], 2)), n_img), [-1, 1])
    ...
    for this_choose in range(5):
        ...
        for fold_j in range(9, 10):
            ...
            for i_save in range(np.size(ei_face, 0)):
                ...
                plt.imsave('./data/eigen/new' + str(fold_j) +"_"+str(i_save) + '.png', (np.reshape(this_eg, [130, 100])))
                a=0
